...
ctrl <- trainControl(
  ...
  horizon = 365,          # or similar
  fixedWindow = FALSE,    # expanding
  skip = 364
)
...
latex_table <- knitr::kable(
  ...
  caption = "Validation performance of Ethereum machine learning models",
...
colnames(test_set_performance_xts) <- c('eth_return','position')

test_set_performance_xts$gross <- test_set_performance_xts$eth_return * test_set_performance_xts$position
...
return_cols <- c("eth_return", "net_ML_model", "net_sma_inv_vol")
...
eth_regression_and_trend_xts <- final_comparison_xts

colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "position"] <- "eth_ml_model_position"
colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "gross"] <- "eth_ml_model_gross"
colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "turnover"] <- "eth_ml_model_turnover"
colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "net_ML_model"] <- "eth_ml_model_net_10bp"


colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "position_sma_inv_vol"] <- "eth_sma_inv_vol_agg_position"
colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "gross_sma_inv_vol"] <- "eth_sma_inv_vol_agg_gross"
colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "net_sma_inv_vol"] <- "eth_sma_inv_vol_agg_net"

eth_regression_and_trend_xts <- merge.xts(eth_regression_and_trend_xts, smas_inverse_vol[,c('turnover')],join = 'inner')

colnames(eth_regression_and_trend_xts)[colnames(eth_regression_and_trend_xts) == "turnover"] <- "eth_sma_inv_vol_agg_turnover"


write.zoo(eth_regression_and_trend_xts, file = "eth_regression_and_trend_xts.csv", sep = ",", row.names = FALSE)
